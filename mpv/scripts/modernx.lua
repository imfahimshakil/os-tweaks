...
function normaliseDate(date)
    ...
    -- Try to parse ISO 8601-like formats (e.g., 2025-10-08T22:11:34.4657800+05:30 or 2025-10-08)
    local year, month, day = date:match("^(%d%d%d%d)%-?(%d%d)?%-?(%d%d)?")
    if year and month and day then
        local dateTable = {year = year, month = month, day = day}
        local success, timestamp = pcall(os.time, dateTable)
        if success then
            return os.date(user_opts.dateformat, timestamp)
        end
    elseif year then
        -- Fallback to year-only if month/day are missing
        return year
    end
